...
function MushraSubcatPage(_pageManager, _pageTemplateRenderer, _audioContext, _bufferSize, _audioFileLoader, _session, _pageConfig, _mushraValidator, _errorHandler, _language) {
	this.isMushra = true; 
  ...
  if (this.pageConfig.has_neutral) {
    this.subcat_names = [{code: "neutral", name: "Neutral"}];
  } else {
    this.subcat_names = [];
  }
  for (var key in this.pageConfig.subcats) {
    this.subcat_names[this.subcat_names.length] = {code: key, name: this.pageConfig.subcats[key]};
  }

  this.scale_names = this.pageConfig.scale;
...
MushraSubcatPage.prototype.getName = function () {
  return this.pageConfig.name;
};

MushraSubcatPage.prototype.init = function () {
   var toDisable;
  var td;
  var active; 
...
MushraSubcatPage.prototype.render = function (_parent) {
  _parent.append('<style>input:checked + span {font-weight: bold;}</style>');
  var div = $("<div></div>");
  _parent.append(div);
  var content; 
  ...
  for (i = 0; i < conditions.length; ++i) {
    ...
    // subcats
  tableMushra.append($('<tr style="height: 1rem;"></tr>'));
  var trConditionSubcats = $("<tr id='tr_ConditionSubcats'></tr>");
  tableMushra.append(trConditionSubcats);

  var tdConditionSubcatsReference = $("<td id='refCanvas'></td>");
  trConditionSubcats.append(tdConditionSubcatsReference);

  var tdConditionSubcatsScale = $("<td id='spaceForScale'></td>");
  trConditionSubcats.append(tdConditionSubcatsScale);
...
MushraSubcatPage.prototype.pause = function() {
    this.mushraAudioControl.pause();
};

MushraSubcatPage.prototype.setLoopStart = function() {
...
MushraSubcatPage.prototype.setLoopEnd = function() {
  var slider = document.getElementById('slider'); 
...
MushraSubcatPage.prototype.btnCallbackReference = function() {
...
MushraSubcatPage.prototype.surpressLoop = function() {
...
MushraSubcatPage.prototype.btnCallbackCondition = function(_index) {
...
MushraSubcatPage.prototype.renderCanvas = function(_parentId) {
  ...
  canvasContext.fillText(this.scale_names[0], xLetters , prevScalesHeight * 0.1 + YfirstLine);
  canvasContext.fillText(this.scale_names[1], xLetters, prevScalesHeight * 0.3 + YfirstLine);
  canvasContext.fillText(this.scale_names[2], xLetters, prevScalesHeight * 0.5 + YfirstLine);
  canvasContext.fillText(this.scale_names[3], xLetters, prevScalesHeight * 0.7 + YfirstLine);
  canvasContext.fillText(this.scale_names[4], xLetters, prevScalesHeight * 0.9 + YfirstLine);
...
MushraSubcatPage.prototype.load = function () {
  ...
  var conditions = this.mushraAudioControl.getConditions();
  var trConditionSubcats = $("#tr_ConditionSubcats");
  for (i = 0; i < conditions.length; ++i) {
    var id = conditions[i].getId()
    td = `<td class='spaceForScale'> \
      <div id='radio-choice-${id}' class='radios' align="left" data-role='controlgroup' data-type='vertical'>`;
    for (var j in this.subcat_names) {
      var code = this.subcat_names[j].code
      var name = this.subcat_names[j].name
      var checked = "";
      if ((this.subcats.length > i && code === this.subcats[i].value) || code === "neutral") {
        checked = " checked"
      } 
      td += `<label for='radio-choice-${code}-${id}'><input type='radio' name='radio-choice-${id}' id='radio-choice-${code}-${id}' value='${code}'${checked}>\
      <span style="font-size: smaller;">${name}</span></label>`;
    }
    if (this.pageConfig.has_other) {
      td += `<label for='radio-choice-other-${id}'><input type='radio' name='radio-choice-${id}' id='radio-choice-other-${id}' value='other'>\
      <span style="font-size: smaller;"><input id='input-other-${id}' type="text" size="1" placeholder="Other" onchange="changeRadioOther('${id}')"/></span></label>`;
    }
    td +="</div></td>"
    td = $(td);
    td.find("input[type='radio']").bind("change", (function(){
      if ($("input:checked").length >= $(".radios").length) {
...
function changeRadioOther(id) {
  var other = document.getElementById(`radio-choice-other-${id}`);
  other.value = document.getElementById(`input-other-${id}`).value;
}

MushraSubcatPage.prototype.save = function () {
  this.macic.unbind(); 
  ...
  for (i = 0; i  < scales.length; ++i) {
    ...
    var subcat = 'no_selected';
    var checked_subcats = $('input[name='+radios[i].id+']:checked');
    if (checked_subcats.length != 0) {
      subcat = checked_subcats[0].value;
    }
    this.subcats[i] = {name: radios[i].id, value: subcat};
  }
...
MushraSubcatPage.prototype.store = function () {
  ...
  for (i = 0; i  < this.ratings.length; ++i) {
    ...
    var subcat = this.subcats[i];
    console.assert("radio-choice-"+rating.name === subcat.name, "Condition names of the rating and the subcategory do not match. Contact the owner of this questionnaire.");
    var ratingObj = new MUSHRARatingSubcat();
    ratingObj.stimulus = rating.name;
    ratingObj.score = rating.value;
    ratingObj.subcat = subcat.value;
